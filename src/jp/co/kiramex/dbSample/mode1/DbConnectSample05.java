package jp.co.kiramex.dbSample.mode1;
...
public class DbConnectSample05 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 4. DBとやりとりする窓口（PreparedStatementオブジェクト）の作成　// ← 修正
            // 検索用SQLおよび検索用PreparedStatementオブジェクトを取得
            String selectsql = "SELECT * FROM city where CountryCode = ?"; // ← 修正
            spstmt = con.prepareStatement(selectsql); // ← 修正

            // 更新するCountryCodeを入力
            System.out.print("検索キーワードを入力してください > ");// ← 追記
            String str1 = keyIn();

            // 入力されたCountryCodeをPreparedStatementオブジェクトにセット  // ← 修正
            spstmt.setString(1, str1); // ← 修正
